...
// New creates a Metal backend for iOS. metalLayer is an
// unsafe.Pointer to a CAMetalLayer. dpiScale is UIScreen.scale.
func New(metalLayer unsafe.Pointer, dpiScale float32) (*Backend, error) {
	if dpiScale <= 0 {
		dpiScale = 1.0
	}
	g, err := gpuInitGo(metalLayer, dpiScale)
	if err != nil {
		return nil, err
	}
	return &Backend{
